...
void ProfileLobbyWidget::setupDuelSetupPanel(QBoxLayout* rootLayout) {
    ...
    connect(opponentModePicker_, &QComboBox::currentTextChanged, this, [this](const QString& text) {
        ...
        opponentPicker_->setVisible(text == "Manual");
        refreshLobbyContext();
    });
...
>>>>>>> 8b838c8 (editing ProfileLobbyWidget.cpp for 1v1 +minor edits for ProfileLobbyWidget.h)
        }
...
void ProfileLobbyWidget::refreshLobbyContext() {
    ...
    const bool isDuel = (selectedModeName_ == "1v1 Fight (Duel)");

    if (duelSetupPanel_) {
        duelSetupPanel_->setVisible(isDuel);
    }
    if (isDeul){
    if (actionHintLabel_) {
            actionHintLabel_->setText(
                QString("%1 vs %2 | Arena: %3")
                    .arg(duelSetup_.opponentCategory)
                    .arg(opponentText)
                    .arg(arenaText)
            );
        }
        ...
        if (previewHintLabel_) {
            previewHintLabel_->setText(
                QString("Duel setup ready: %1 | %2")
                    .arg(opponentText)
                    .arg(arenaText)
            );
        }
        ...
        if (enterArenaButton_) {
            enterArenaButton_->setText("START DUEL");
        }
        return;
    }
