  ...
  QString characterTypeToString(CharacterType type)
{
  ...
  void CombatAiAdvisor::requestRecommendation(const CombatSnapshot& snapshot)
  {
    ...
    QString userPrompt = buildUserPrompt(snapshot);

    client_->sendMessage(systemPrompt, userPrompt, [this, snapshot](const QByteArray& response){
        AiRecommendation recommendation = parseRecommendation(response, snapshot);
